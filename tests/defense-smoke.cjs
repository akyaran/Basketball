...
assert.equal(result.threeSeconds.inboundX, 771);
assert.ok([-18, 838].includes(result.threeSeconds.inboundY));
...
assert.equal(result.shotClockInbound.y, -18);
...
assert.equal(result.twoPointNoCelebration.ballStart.x, 1560);
