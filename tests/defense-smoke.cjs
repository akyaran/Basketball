...
resolveStealTiming();
Math.random = originalRandom;
globalThis.testResult.playerFoul = { possession: state.possession, inbound: Boolean(state.inbound), shotClock: state.shotClock };
updateSideInbound(2);
const cpuInboundPass = { owner: state.passBall?.owner, blocked: state.passBall?.interceptions.length === 0 };
updatePassBall(1);
globalThis.testResult.cpuInbound = { ...cpuInboundPass, inbound: state.inbound, handler: state.cpuHandler };
setPossession("player");
player.x = 600;
player.y = 410;
defender.x = 648;
...
Math.random = originalRandom;
globalThis.testResult.cpuFoul = { cpuFouled, possession: state.possession, inbound: Boolean(state.inbound), shotClock: state.shotClock };
settings.players = "1v1";
setPossession("player");
beginSideInbound("player", { x: 600, y: 300 });
updateSideInbound(2);
globalThis.testResult.oneOnOneInbound = { inbound: state.inbound, possession: state.possession, handler: state.playerHandler };
`;
...
assert.equal(result.playerFoul.inbound, true);
assert.ok(result.playerFoul.shotClock >= 14);
assert.equal(result.cpuInbound.owner, "cpu");
assert.equal(result.cpuInbound.blocked, true);
assert.equal(result.cpuInbound.inbound, null);
...
assert.equal(result.cpuFoul.inbound, true);
assert.ok(result.cpuFoul.shotClock >= 14);
assert.equal(result.oneOnOneInbound.inbound, null);
assert.equal(result.oneOnOneInbound.possession, "player");
assert.equal(result.oneOnOneInbound.handler, "player");
